...
import Edit from "@/Pages/Comment/Edit.vue";
import Delete from "@/Pages/Comment/Delete.vue";
import DeleteBulk from "@/Pages/Comment/DeleteBulk.vue";
...
watch(
    ...
    debounce(() => {
        ...
        router.get(route("comment.index"), params, {
            replace: true,
            preserveState: true,
            preserveScroll: true,
        });
...
const selectAll = (event) => {
    if (event.target.checked === false) {
        ...
        props.comments?.data.forEach((comment) => {
            data.selectedId.push(comment.id);
        });
    }
};
const select = () => {
    if (props.comments?.data.length == data.selectedId.length) {
                        ...
                        <template #table-body>
                            ...
                            >
                                ...
                                <td class="whitespace-nowrap flex justify-end px-4 py-2">
                                    ...
                                    >
                                        <Edit
                                            v-show="can(['comment update'])"
                                            :title="props.title"
                                            :comment="data.comment"
                                            @open="data.comment = comment"
                                        />
                                        <Delete
